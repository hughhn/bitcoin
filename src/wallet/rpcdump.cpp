...
static UniValue ProcessDescriptorImport(CWallet * const pwallet, const UniValue& data, const int64_t timestamp) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    try {
        ...
        const std::string& descriptor = data["desc"].get_str();
        const bool active = data.exists("active") ? data["active"].get_bool() : true;
        const bool internal = data.exists("internal") ? data["internal"].get_bool() : false;
        const std::string& label = data.exists("label") ? data["label"].get_str() : "";
        const std::string& output_param = data.exists("address_type") ? data["address_type"].get_str() : "";

        OutputType output_type = pwallet->m_default_address_type;
        if (!ParseOutputType(output_param, output_type)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, strprintf("Unknown address type '%s'", output_param));
        }

        // Active descriptor must have a range
        if (active && !data.exists("range")) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Active descriptor must have a range");
        }
        ...
        // If the wallet disabled private keys, abort if private keys are being imported
        ...
        WalletDescriptor w_desc(std::move(parsed_desc), timestamp, range_start, range_end, range_start);

        // Check if the wallet already contains the descriptor
        if (pwallet->HasWalletDescriptor(w_desc)) {
            throw JSONRPCError(RPC_WALLET_ERROR, strprintf("The wallet already has descriptor '%s'", descriptor));
        }
        ...
        if (active) {
            pwallet->SetActiveScriptPubKeyMan(spk_manager->GetID(), output_type, internal);
        }
...
UniValue importdescriptors(const JSONRPCRequest& main_request) {
    ...
    if (main_request.fHelp || main_request.params.size() < 1 || main_request.params.size() > 2)
                            ...
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                                    ...
                                    {"address_type", RPCArg::Type::STR, /* default */ "set by address_type", "The address type to use. Options are \"legacy\", \"p2sh-segwit\", and \"bech32\"."},
        ...
        "                                                              of all descriptors being imported will be scanned.",
            ...
            "  [{\"success\": true}, {\"success\": true, \"warnings\": [\"Ignoring irrelevant private key\"]}, {\"success\": false, \"error\": {\"code\": -1, \"message\": \"Internal Server Error\"}}, ...]\n"
                ...
                RPCExamples{
                    HelpExampleCli("importdescriptors", "'[{ \"desc\": \"<my descriptor>\", \"timestamp\":1455191478, \"address_type\": \"legacy\", \"internal\": true }, "
                                          "{ \"desc\": \"<my desccriptor 2>\", \"label\": \"example 2\", \"timestamp\": 1455191480, \"address_type\": \"legacy\" }]'") +
                    HelpExampleCli("importdescriptors", "'[{ \"desc\": \"<my descriptor>\", \"timestamp\":1455191478, \"address_type\": \"bech32\", \"active\": true, \"range\": [0,100], \"label\": \"<my bech32 wallet>\" }]'")
                },
            }.ToString()
        );
    ...
    UniValue response(UniValue::VARR);
    {
        ...
        for (const UniValue& request : requests.getValues()) {
            ...
            // TODO: should we throw an error if we couldn't find the tip?
            if (!rescan && tip_height && result["success"].get_bool()) {
                rescan = true;
            }
        }
    }
