...
size_t CWallet::KeypoolCountExternalKeys()
{
    ...
    for (auto spk_man_pair : m_spk_managers) {
        count += spk_man_pair.second->KeypoolCountExternalKeys();
    }
    return count;
}
...
unsigned int CWallet::GetKeyPoolSize() const
{
    ...
    unsigned int count = 0;
    for (auto spk_man_pair : m_spk_managers) {
        count += spk_man_pair.second->GetKeypoolSize();
    }
    return count;
}
...
bool CWallet::TopUpKeyPool(unsigned int kpSize)
{
    ...
    bool res = true;
    for (OutputType t : output_types) {
        res &= GetScriptPubKeyMan(t, false)->TopUp(kpSize);
        res &= GetScriptPubKeyMan(t, true)->TopUp(kpSize);
    }
    return res;
}
...
bool CWallet::GetNewDestination(const OutputType type, const std::string label, CTxDestination& dest, std::string& error)
{
    LOCK(cs_wallet);
    error.clear();
    bool result = GetScriptPubKeyMan(type, false)->GetNewDestination(type, dest, error);
    if (result) {
...
std::shared_ptr<CWallet> CWallet::CreateWalletFromFile(interfaces::Chain& chain, const WalletLocation& location, uint64_t wallet_creation_flags)
{
    ...
    if (tip_height && *tip_height != rescan_height)
    {
        ...
        for (OutputType t : output_types) {
            std::min(time_first_key, walletInstance->GetScriptPubKeyMan(t, false)->GetTimeFirstKey());
            std::min(time_first_key, walletInstance->GetScriptPubKeyMan(t, true)->GetTimeFirstKey());
        }
        if (time_first_key) {
